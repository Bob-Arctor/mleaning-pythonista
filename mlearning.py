...
def regress(samples, target, method='linearGD', iterations=20, rate=0.01, precision=6):
	# regression
	# linear with gradient decent or exact formula
	# or logistic
	# param method: 'logistic',  'linearGD' - gradient descent, 'linearEX' - using formula
	...
	if method=='linearEX':
	...
	elif method in ('logistic', 'linearGD'):
		# calculate h theta
		h = np.dot(samples, weights)
		rate = float(rate) / samples.shape[0] 
		...
		for i in range(iterations):
			...
			old_err = err
			# calculate gradient
			grad = np.dot(np.transpose(h - target), samples) * rate
			# adjust weights
...
def logreg(samples, target, iterations=20, rate=0.01, precision=6):
	# logistic regression
	# first init weights matrix - dimention m + 1 where m is dimention of samples
	# adding extra element for x0 = 1
	weights = np.zeros([samples.shape[1] + 1, 1])
